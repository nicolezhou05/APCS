 ...
 *
 * ALGO
 * 0) If tour is completed, everything stops and ends.
 * 1) Uses recursion to create the tour.
 * DISCO
 * 0) Having extra layers outside the board helps with accidental errors.
 * 1) There could be multiple classes in one file. One needs to have "public" in front, and it is also the name of the file.
 * QCC
 * 0) When is having multiple classes in one file better than having it in each individual file?
 * 1) There are a few weird symbols in this file. What are they used for?
 * 2) What are the differences between the different times when using the time program?
 *
...
class TourFinder{
  ...
  public void findTour( int x, int y, int moves ){
    ...
    if ((x < 2) || (x > _board.length - 3) || (y < 2) || (y > _board.length - 3) || _board[x][y] > 0) {
      return;
    }
