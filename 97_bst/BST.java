...
public class BST
{
  ...
  public void remove(int value){
    TreeNode node = search(value); // the node we're removing
    if (isLeaf(node)){
      TreeNode parent = getParent(_root, node);
      ...
      node.setValue(noLeftSubtree(node));
    }
    else{
      node.setValue(noRightSubtree(node));
    }
  }

  public TreeNode getParent(TreeNode currNode, TreeNode child){
    if (currNode.getValue() > child.getValue()){
      if (currNode.getLeft() == child){
        return currNode;
      }
      return getParent(currNode.getLeft(), child);
    }
    else{
      if (currNode.getRight() == child){
        return currNode;
      }
      return getParent(currNode.getRight(), child);
    }
  }

  public int noLeftSubtree(TreeNode currNode){
  ...
  public int noRightSubtree(TreeNode currNode){
    currNode = currNode.getLeft();
